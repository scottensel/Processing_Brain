...
# June 2022
#
# Preparation of functional brain data
#
# Requirements: FSL
#
#
...
for s in "${sub[@]}"; do
        ...
        for d in "${myFunc[@]}"; do
			...
			# 2 - GENERATE EVS
                ...
                # creating EVs (regressors) to model the physiological noise, of cardiac and respiration  #--oc=4 --or=4 --multc=2 --multr=2
                $FSLDIR/bin/pnm_evs -i $DIREC$s"/func/func"$d"/fmri_brain_moco_reg.nii.gz" -c $s"_card.txt" -r $s"_resp.txt" -o $s --tr=2.2 --oc=4 --or=4 --multc=2 --multr=2  --csfmask=$DIREC$s"/func/func1/MNI152_CSF.nii.gz" --sliceorder=interleaved_up --slicetiming=slice_times.txt -v
