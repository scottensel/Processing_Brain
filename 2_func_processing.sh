...
tput setaf 6; 
echo -n "Enter the index of the step to perform (1 = Crop the EPI, 2 = slice timing & Moco, 3 = distortion correction, 4 = Registration to first run): "
tput sgr0;
read ind
...
for s in "${sub[@]}"; do
        ...
        for d in "${myFunc[@]}"; do
			...
			if [ "$ind" == "1" ]; then
                ...
                slicetimer -i fmri_crop.nii.gz -o fmri_crop.nii.gz -r 2.2 --tcustom=../../physio/physio"$d"/slice_order.txt -v

                # motion correction
                mcflirt -in fmri_crop.nii.gz -out fmri_moco -refvol 0 -plots -report
