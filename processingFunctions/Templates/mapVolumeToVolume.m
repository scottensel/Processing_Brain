function Vout=mapVolumeToVolume(Vin_i,Vout_fn)
...
% Vout is filled with zeros and has the size of the output file
Vout=zeros(Vout_i.dim);

% Thomas (Feb 2016): had to change this for the shared scripts to work
% properly (they ask for Vin_i to be a file path, not a NIFTI header, so I
% added the spm_vol line below)
Vin_i = spm_vol(Vin_i);

% Vin contains the input data (3D matrix)
